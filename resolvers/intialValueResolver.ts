...
export const resolveIntialValueRoot = async (
  dataSources: DataSources,
  parent: any,
  key: string,
  formatter: string | null,
  ...
  try {
    ...
    return formatterFactory(ResolverFormatters.Root)({
      value: value ?? '',
      formatter,
      formatterSettings,
      entity,
...
export const resolveIntialValueRelations = async (
  ...
  try {
    const relations =
      parent?.relations?.filter((relation: any) => relation.type === key) || [];
...
export const resolveIntialValueMetadataOrRelation = async (
  ...
  try {
    ...
    const metadata = await resolveIntialValueMetadata(
      dataSources,
      parent,
      metadataKey,
      undefined,
...
export const resolveIntialValueParentRoot = async (
  ...
  const finalParent = await loopOverParentRelations(
    dataSources,
    parent,
    parentRelations
  );
...
export const resolveIntialValueParentRelations = async (
  ...
  const relations =
    finalParent?.relations?.filter((relation: any) => relation.type === key) ||
    [];
...
const loopOverParentRelations = async (
  ...
  for (const parentRelation of parentRelations) {
    ...
    if (parentRelation.relationType) {
      let id = getIdThroughCurrentRelations(
        currentParent,
      ...
      currentParent = await getParentTroughFilter(
        dataSources,
        currentParent,
        parentRelation
      );
...
const getIdThroughCurrentRelations = (
  currentParent: any,
  relationType: string
): string | undefined => {
  const relations =
    currentParent?.relations?.filter(
