...
export const baseEnvironment: Environment = {
  apollo: {
    ...
    // "0" must survive parsing (it disables the depth rule), so no || here
    maxQueryDepth: process.env.APOLLO_MAX_QUERY_DEPTH
      ? Number(process.env.APOLLO_MAX_QUERY_DEPTH)
      : 15,
  },
  ...
  glitchtipEnabled: process.env.GLITCH_TIP_ENABLED === 'true',
  glitchtipDsn: process.env.GLITCH_TIP_DSN || '',
  glitchtipDsnFrontend: process.env.GLITCH_TIP_DSN_FRONTEND || '',
