...
const returnEntityInFormat = async (
  ...
  try {
    ...
    res
      ...
      .setHeader('Content-Type', acceptHeadersForFormat[format]);
...
const isEntityPresent = async (id: string, req: Request): Promise<boolean> => {
  const env: Environment = getCurrentEnvironment();
  const headers = {
    authorization: req.headers.authorization as string,
  };
  const response = await fetch(`${env.api.collectionApiUrl}entities/${id}`, {
    headers,
  });
  ...
  app.get('*', async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (shouldServeFrontend(format)) {
        ...
        if (await isEntityPresent(entityId, req))
          await returnEntityInFormat(entityId, format!, req, res);
