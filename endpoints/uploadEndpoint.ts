...
export const applyUploadEndpoint = (app: Express) => {
    ...
    app.post(
        ...
        async (request: Request, response: Response) => {
            try {
                const entityInput = request.body.entityInput ?? { metadata: [], relations: [] };
                if (request.query?.hasRelation) {
                    const uploadUrl = await __createMediafileForEntity(request, entityInput);
                    response.end(JSON.stringify(uploadUrl));
                } else {
                    const uploadUrl = await __createStandaloneMediafile(request, entityInput);
                    response.end(JSON.stringify(uploadUrl));
                }
            } catch (exception: any) {
                response
